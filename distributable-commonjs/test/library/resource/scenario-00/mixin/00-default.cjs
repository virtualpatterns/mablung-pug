'use strict'

Object.defineProperty(exports, '__esModule', {
  value: true
})
exports.default = _default

var _utility = require('../../../../../library/utility.cjs')

// Created by @virtualpatterns/mablung-pug v0.0.1-18
// Created at 2020-11-10 05:31:28
// Path = 'distributable-commonjs/test/library/resource/scenario-00/mixin/00-default.pug'
function __getNode(__local = {}, __utility = {}) {
  function __getNode(__utility = {}) {
    const __node = [] // Declaration

    function __mixin__list(attribute, block) {
      const attributes = attribute
      const __node = []

      __utility.addNode(
        __utility.createNode(
          'ul',
          {},
          (() => {
            const __node = []

            __utility.addNode(
              __utility.createNode(
                'li',
                {},
                (() => {
                  const __node = []

                  __utility.addNode(__utility.convertToNode('foo'), __node)

                  return __node
                })()
              ),
              __node
            )

            __utility.addNode(
              __utility.createNode(
                'li',
                {},
                (() => {
                  const __node = []

                  __utility.addNode(__utility.convertToNode('bar'), __node)

                  return __node
                })()
              ),
              __node
            )

            __utility.addNode(
              __utility.createNode(
                'li',
                {},
                (() => {
                  const __node = []

                  __utility.addNode(__utility.convertToNode('baz'), __node)

                  return __node
                })()
              ),
              __node
            )

            return __node
          })()
        ),
        __node
      )

      return __node
    } // Use

    __utility.addNode(
      __utility.createNode(
        'div',
        {},
        (() => {
          const __node = []

          __utility.addNode(__mixin__list(undefined, undefined), __node)

          __utility.addNode(__mixin__list(undefined, undefined), __node)

          return __node
        })()
      ),
      __node
    )

    return __utility.getNode(__node)
  }

  return __getNode(__utility)
}

function _default(__local = {}, __utility = _utility.Utility) {
  return __getNode(__local, __utility)
}
